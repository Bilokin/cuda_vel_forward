...
// 数学常量
__constant__ double c1 = -2.5f;
__constant__ double c2 = 4.0f / 3.0f;
__constant__ double c3 = -1.0f / 12.0f;

// 数值稳定性常量
__constant__ double MAX_VALUE = 1e8f;  // 最大允许值
__constant__ double MIN_VALUE = 1e-8f; // 最小有意义值

// 预计算 damp1d 数组的 kernel
__global__ void compute_damp1d_kernel(double* damp1d, double kappa, double dx, double a, int nbc) {
...
__global__ void abc_coef_kernel(double* abc, const double* damp1d, int nz_pad, int nx_pad, 
                               int nbc) {
...
__global__ void laplacian_kernel(double* lap, const double* p, int nz_pad, int nx_pad) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int idy = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    // 预取数据以减少内存延迟并进行数值检查
    double p_y_plus1 = p[idx * nx_pad + y_plus1];
    double p_y_minus1 = p[idx * nx_pad + y_minus1];
    double p_x_plus1 = p[x_plus1 * nx_pad + idy];
    double p_x_minus1 = p[x_minus1 * nx_pad + idy];
    
    double p_y_plus2 = p[idx * nx_pad + y_plus2];
    double p_y_minus2 = p[idx * nx_pad + y_minus2];
    double p_x_plus2 = p[x_plus2 * nx_pad + idy];
    double p_x_minus2 = p[x_minus2 * nx_pad + idy];
    ...
    double term1 = c2 * (p_y_plus1 + p_y_minus1 + p_x_plus1 + p_x_minus1);
    double term2 = c3 * (p_y_plus2 + p_y_minus2 + p_x_plus2 + p_x_minus2);
    
    double lap_val = term1 + term2;
...
// 波场更新 kernel
__global__ void wavefield_update_kernel(double* p_new, double* p_cur, double* p_old,
                                       const double* lap, const double* alpha, 
                                       const double* kappa, int nz_pad, int nx_pad) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int idy = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    double alpha_val = alpha[center];
    double kappa_val = kappa[center];
    double lap_val = lap[center];
    double p_cur_val = p_cur[center];
    double p_old_val = p_old[center];
    ...
    double t1 = 2.0f + 2.0f * c1 * alpha_val - kappa_val;
    double t2 = 1.0f - kappa_val;
    
    // 分段计算减少数值误差
    double term1 = t1 * p_cur_val;
    double term2 = t2 * p_old_val;
    double term3 = alpha_val * lap_val;
...
__global__ void source_inject_kernel(double* p, int sz, int sx, double source_val, 
                                    int nx_pad) {
    // 数值范围检查
    ...
    // 检查当前波场值
    double current_val = p[pos];
    if (!isfinite(current_val)) {
        p[pos] = source_val; // 如果当前值无效，直接设置
    } else {
        double new_val = current_val + source_val;
        // 检查结果是否有效
...
// 改进的自由表面边界条件 kernel
__global__ void free_surface_kernel(double* p, int nbc, int nx_pad) {
    ...
    if (nbc >= 1) {
        ...
        double val_above1 = p[pos_above1];
    ...
    if (nbc >= 2) {
        ...
        double val_above2 = p[pos_above2];
...
// 记录提取 kernel
__global__ void record_extract_kernel(double* seis, const double* p, 
...
void cuda_acoustic_forward_precomputed(torch::Tensor vpad,           // (H_pad, W_pad) 
    ...
    // 获取设备和数据类型选项
    auto options = torch::TensorOptions().dtype(torch::kFloat64).device(vpad.device());
    
    int H_pad = vpad.size(0);  
    int W_pad = vpad.size(1);  
    
    // 计算吸收边界系数 - 按照 Python AbcCoef2D 实现
    double velmin = vpad.min().item<double>();
    auto abc = torch::zeros({H_pad, W_pad}, options);
    
    if (nbc > 1) {
        // 计算 kappa 和 damp1d
        double a = (nbc - 1) * dx;
        double kappa = 3.0f * velmin * logf(1e7f) / (2.0f * a);
        ...
        compute_damp1d_kernel<<<grid_1d, block_1d>>>(
            damp1d.data_ptr<double>(), kappa, dx, a, nbc);
        ...
        abc_coef_kernel<<<grid_2d, block_2d>>>(
            abc.data_ptr<double>(), damp1d.data_ptr<double>(),
            H_pad, W_pad, nbc);
    }
    ...
    for (int src_idx = 0; src_idx < n_src; src_idx++) {
        ...
        for (int it = 0; it < nt; it++) {
            ...
            laplacian_kernel<<<grid_2d, block_2d>>>(
                lap.data_ptr<double>(), p_cur.data_ptr<double>(),
                H_pad, W_pad);
            
            // 更新波场
            wavefield_update_kernel<<<grid_2d, block_2d>>>(
                p_new.data_ptr<double>(), p_cur.data_ptr<double>(),
                p_old.data_ptr<double>(), lap.data_ptr<double>(),
                alpha.data_ptr<double>(), kappa.data_ptr<double>(),
                H_pad, W_pad);
            
            // 注入震源
            double source_val = beta_d[isz_val][sx].item<double>() * wavelet[it].item<double>();
            source_inject_kernel<<<1, 1>>>(
                p_new.data_ptr<double>(), isz_val, sx, source_val, W_pad);
            
            // 自由表面边界条件
            if (isFS) {
                dim3 grid_fs((W_pad + 255) / 256);
                free_surface_kernel<<<grid_fs, 256>>>(
                    p_new.data_ptr<double>(), nbc, W_pad);
            }
            ...
            record_extract_kernel<<<grid_rec, 256>>>(
                seis_slice.select(0, it).data_ptr<double>(), p_new.data_ptr<double>(),
...
void cuda_acoustic_forward_batch(torch::Tensor vpad_batch,       // (B, H_pad, W_pad)
    ...
    // 获取设备和数据类型选项
    auto options = torch::TensorOptions().dtype(torch::kFloat64).device(vpad_batch.device());
    ...
    for (int b = 0; b < B; b++) {
        ...
        // 计算吸收边界系数
        double velmin = vpad_current.min().item<double>();
        abc.zero_();
        
        if (nbc > 1) {
            // 计算 kappa 和 damp1d
            double a = (nbc - 1) * dx;
            double kappa_val = 3.0f * velmin * logf(1e7f) / (2.0f * a);
            ...
            compute_damp1d_kernel<<<grid_1d, block_1d, 0, current_stream>>>(
                damp1d.data_ptr<double>(), kappa_val, dx, a, nbc);
